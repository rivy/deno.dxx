 ...
 */
function createColor(diffType: DiffType, { background = false } = {}): (s: string) => string {
    switch (diffType) {
        case DiffType.added:
            return (s: string): string => background ? bgGreen(white(s)) : green(bold(s));
...
function buildMessage(diffResult: ReadonlyArray<DiffResult<string>>, { stringDiff = false } = {}): string[] {
 ...
 * ```
 */
export function assertAlmostEquals(actual: number, expected: number, tolerance = 1e-7, msg?: string) {
    if (actual === expected) {
        return;
    }
 ...
 * If not then throw.
 */
export function assertInstanceOf<T extends AnyConstructor>(actual: unknown, expectedType: T, msg = ""): asserts actual is GetConstructorType<T> {
    if (!msg) {
 ...
 * ```
 */
export function assertArrayIncludes(actual: ArrayLike<unknown>, expected: ArrayLike<unknown>, msg?: string): void;
...
// deno-lint-ignore no-explicit-any
actual: Record<PropertyKey, any>, expected: Record<PropertyKey, unknown>): void {
 ...
 */
export function assertIsError<E extends Error = Error>(error: unknown, 
// deno-lint-ignore no-explicit-any
ErrorClass?: new (...args: any[]) => E, msgIncludes?: string, msg?: string): asserts error is E {
 ...
 * assertions on it.
 */
export function assertThrows<E extends Error = Error>(fn: () => unknown, 
// deno-lint-ignore no-explicit-any
ErrorClass?: new (...args: any[]) => E, msgIncludes?: string, msg?: string): void;
export function assertThrows(fn: () => unknown, errorCallback: (e: Error) => unknown, msg?: string): void;
export function assertThrows<E extends Error = Error>(fn: () => unknown, errorClassOrCallback?: 
// deno-lint-ignore no-explicit-any
(new (...args: any[]) => E) | ((e: Error) => unknown), msgIncludesOrMsg?: string, msg?: string): void {
    // deno-lint-ignore no-explicit-any
 ...
 * assertions on it.
 */
export function assertRejects<E extends Error = Error>(fn: () => Promise<unknown>, 
// deno-lint-ignore no-explicit-any
ErrorClass?: new (...args: any[]) => E, msgIncludes?: string, msg?: string): Promise<void>;
export function assertRejects(fn: () => Promise<unknown>, errorCallback: (e: Error) => unknown, msg?: string): Promise<void>;
export async function assertRejects<E extends Error = Error>(fn: () => Promise<unknown>, errorClassOrCallback?: 
// deno-lint-ignore no-explicit-any
(new (...args: any[]) => E) | ((e: Error) => unknown), msgIncludesOrMsg?: string, msg?: string): Promise<void> {
    // deno-lint-ignore no-explicit-any
    let ErrorClass: (new (...args: any[]) => E) | undefined = undefined;
    let msgIncludes: string | undefined = undefined;
    let errorCallback;
