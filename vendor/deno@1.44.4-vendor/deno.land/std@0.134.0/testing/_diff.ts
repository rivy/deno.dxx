...
export function diff<T>(A: T[], B: T[]): Array<DiffResult<T>> {
    ...
    if (!N) {
        return [
            ...prefixCommon.map((c): DiffResult<typeof c> => ({ type: DiffType.common, value: c })),
            ...A.map((a): DiffResult<typeof a> => ({
                type: swapped ? DiffType.added : DiffType.removed,
                value: a,
            })),
            ...suffixCommon.map((c): DiffResult<typeof c> => ({ type: DiffType.common, value: c })),
        ];
    }
    const offset = N;
    const delta = M - N;
    const size = M + N + 1;
    const fp: FarthestPoint[] = Array.from({ length: size }, () => ({ y: -1, id: -1 }));
    /**
     * INFO:
...
export function diffstr(A: string, B: string) {
    ...
    function createDetails(line: DiffResult<string>, tokens: Array<DiffResult<string>>) {
    ...
    for (const a of aLines) {
        let tokens = [] as Array<DiffResult<string>>, b: undefined | DiffResult<string>;
