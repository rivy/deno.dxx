...
import { GlobOptions, globToRegExp, isAbsolute, isGlob, joinGlobs, resolve, SEP_PATTERN, } from "../path/mod.ts";
import { _createWalkEntry, _createWalkEntrySync, walk, WalkEntry, walkSync, } from "./walk.ts";
 ...
 * ```
 */
export async function* expandGlob(glob: string, { root = Deno.cwd(), exclude = [], includeDirs = true, extended = true, globstar = false, caseInsensitive, }: ExpandGlobOptions = {}): AsyncIterableIterator<WalkEntry> {
    ...
    async function* advanceMatch(walkInfo: WalkEntry, globSegment: string): AsyncIterableIterator<WalkEntry> {
 ...
 * ```
 */
export function* expandGlobSync(glob: string, { root = Deno.cwd(), exclude = [], includeDirs = true, extended = true, globstar = false, caseInsensitive, }: ExpandGlobOptions = {}): IterableIterator<WalkEntry> {
    const globOptions: GlobOptions = { extended, globstar, caseInsensitive };
    const absRoot = resolve(root);
    const resolveFromRoot = (path: string): string => resolve(absRoot, path);
    const excludePatterns = exclude
    ...
    function* advanceMatch(walkInfo: WalkEntry, globSegment: string): IterableIterator<WalkEntry> {
        if (!walkInfo.isDirectory) {
            return;
        }
        else if (globSegment == "..") {
