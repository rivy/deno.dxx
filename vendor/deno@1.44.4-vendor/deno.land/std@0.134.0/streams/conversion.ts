 ...
 * ```
 */
export function readerFromIterable(iterable: Iterable<Uint8Array> | AsyncIterable<Uint8Array>): Deno.Reader {
    const iterator: Iterator<Uint8Array> | AsyncIterator<Uint8Array> = (iterable as AsyncIterable<Uint8Array>)[Symbol.asyncIterator]?.() ??
...
export function writerFromStreamWriter(streamWriter: WritableStreamDefaultWriter<Uint8Array>): Deno.Writer {
    return {
...
export function readerFromStreamReader(streamReader: ReadableStreamDefaultReader<Uint8Array>): Deno.Reader {
    const buffer = new Buffer();
    return {
        async read(p: Uint8Array): Promise<number | null> {
            if (buffer.empty()) {
...
export function writableStreamFromWriter(writer: Deno.Writer, options: WritableStreamFromWriterOptions = {}): WritableStream<Uint8Array> {
 ...
 * ```
 */
export function readableStreamFromIterable<T>(iterable: Iterable<T> | AsyncIterable<T>): ReadableStream<T> {
 ...
 * ```
 */
export function readableStreamFromReader(reader: Deno.Reader | (Deno.Reader & Deno.Closer), options: ReadableStreamFromReaderOptions = {}): ReadableStream<Uint8Array> {
