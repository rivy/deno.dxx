...
export function bench(benchmark: BenchmarkDefinition | BenchmarkFunction): void {
    if (!benchmark.name) {
...
export function clearBenchmarks({ only = /[^\s]/, skip = /$^/, }: BenchmarkClearOptions = {}): void {
    const keep = candidates.filter(({ name }): boolean => !only.test(name) || skip.test(name));
 ...
 */
export async function runBenchmarks({ only = /[^\s]/, skip = /^\s*$/, silent }: BenchmarkRunOptions = {}, progressCb?: (progress: BenchmarkRunProgress) => void | Promise<void>): Promise<BenchmarkRunResult> {
    // Filtering candidates by the "only" and "skip" constraint
    const benchmarks: BenchmarkDefinition[] = candidates.filter(({ name }): boolean => only.test(name) && !skip.test(name));
    ...
    if (!silent) {
        // Closing results
        console.log(`benchmark result: ${failError ? red("FAIL") : blue("DONE")}. ` +
...
async function publishProgress(progress: BenchmarkRunProgress, state: ProgressState, progressCb?: (progress: BenchmarkRunProgress) => void | Promise<void>): Promise<void> {
    progressCb && (await progressCb(cloneProgressWithState(progress, state)));
}
function cloneProgressWithState(progress: BenchmarkRunProgress, state: ProgressState): BenchmarkRunProgress {
