...
import type { IFlagArgument, IFlagOptions, IFlagsResult, IParseOptions, ITypeHandler, } from "./types.ts";
...
O extends Record<string, any> = Record<string, any>, T extends IFlagOptions = IFlagOptions>(args: string[], opts: IParseOptions<T> = {}): IFlagsResult<O> {
    args = args.slice();
    ...
    for (let argsIndex = 0; argsIndex < args.length; argsIndex++) {
        ...
        if (isFlag) {
            ...
            function parseNext(option: IFlagOptions, optionArgs: IFlagArgument[]): void {
                ...
                function parseValue(option: IFlagOptions, arg: IFlagArgument, value: string): unknown {
    ...
    const result = Object.keys(flags)
        ...
        if (~key.indexOf(".")) {
            key.split(".").reduce((
            // deno-lint-ignore no-explicit-any
            result: Record<string, any>, subKey: string, index: number, parts: string[]) => {
...
function parseFlagValue(option: IFlagOptions, arg: IFlagArgument, value: string): unknown {
    const type: string = arg.type || OptionType.STRING;
