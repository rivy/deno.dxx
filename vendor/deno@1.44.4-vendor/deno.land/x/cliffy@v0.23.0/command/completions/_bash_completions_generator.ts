...
complete -F _${replaceSpecialChars(path)} -o bashdefault -o default ${path}`;
    ...
    private generateCompletions(command: Command, path = "", index = 1): string {
        ...
        const childCommandCompletions: string = command.getCommands(false)
            ...
            .map((subCommand: Command) => this.generateCompletions(subCommand, path, index + 1))
            .join("");
        return `${commandCompletions}

${childCommandCompletions}`;
    }
    private generateCommandCompletions(command: Command, path: string, index: number): string {
        ...
        const completionsCmd: string = this.generateCommandCompletionsCommand(command, completionsPath);
    ...
    private generateOptionArguments(command: Command, completionsPath: string): string {
        let opts = "";
        ...
        if (options.length) {
            ...
            for (const option of options) {
                const flags: string = option.flags
                    ...
                    .join("|");
                const completionsCmd: string = this.generateOptionCompletionsCommand(command, option.args, completionsPath, { standalone: option.standalone });
    ...
    private generateOptionCompletionsCommand(command: Command, args: IArgument[], path: string, opts?: {
        standalone?: boolean;
    }) {
        if (args.length) {
            const type = command.getType(args[0].type);
