        ...
        if (type instanceof Type) {
            ...
            if (possibleValues?.length) {
                hints.push(bold(`Values: `) +
                    possibleValues.map((value: unknown) => inspect(value, this.options.colors)).join(", "));
            }
        }
        if (hints.length) {
            return `(${hints.join(", ")})`;
...
function highlightArguments(argsDefinition: string, types = true) {
    ...
    return parseArgumentsDefinition(argsDefinition, false, true)
        .map((arg: IArgument | string) => typeof arg === "string" ? arg : highlightArgumentDetails(arg, types))
        .join(" ");
}
/**
