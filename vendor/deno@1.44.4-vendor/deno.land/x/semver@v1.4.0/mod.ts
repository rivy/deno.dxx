...
export function parse(version: string | SemVer | null, optionsOrLoose?: boolean | Options): SemVer | null {
...
export function valid(version: string | SemVer | null, optionsOrLoose?: boolean | Options): string | null {
    if (version === null)
        return null;
...
export function clean(version: string, optionsOrLoose?: boolean | Options): string | null {
    const s: SemVer | null = parse(version.trim().replace(/^[=v]+/, ""), optionsOrLoose);
...
export function diff(version1: string | SemVer, version2: string | SemVer, optionsOrLoose?: boolean | Options): ReleaseType | null {
...
export function compareIdentifiers(a: string | number | null, b: string | number | null): 1 | 0 | -1 {
...
export function compareLoose(a: string | SemVer, b: string | SemVer): 1 | 0 | -1 {
    return compare(a, b, true);
}
export function compareBuild(a: string | SemVer, b: string | SemVer, loose?: boolean | Options): 1 | 0 | -1 {
...
export class Comparator {
    ...
    intersects(comp: Comparator, optionsOrLoose?: boolean | Options): boolean {
        ...
        const sameDirectionIncreasing: boolean = (this.operator === ">=" || this.operator === ">") &&
            (comp.operator === ">=" || comp.operator === ">");
        const sameDirectionDecreasing: boolean = (this.operator === "<=" || this.operator === "<") &&
            (comp.operator === "<=" || comp.operator === "<");
        const sameSemVer: boolean = this.semver.version === comp.semver.version;
        const differentDirectionsInclusive: boolean = (this.operator === ">=" || this.operator === "<=") &&
            (comp.operator === ">=" || comp.operator === "<=");
        const oppositeDirectionsLessThan: boolean = cmp(this.semver, "<", comp.semver, optionsOrLoose) &&
            (this.operator === ">=" || this.operator === ">") &&
            (comp.operator === "<=" || comp.operator === "<");
        const oppositeDirectionsGreaterThan: boolean = cmp(this.semver, ">", comp.semver, optionsOrLoose) &&
            (this.operator === "<=" || this.operator === "<") &&
            (comp.operator === ">=" || comp.operator === ">");
...
function testSet(set: ReadonlyArray<Comparator>, version: SemVer, options: Options): boolean {
...
function isSatisfiable(comparators: readonly Comparator[], options?: boolean | Options): boolean {
...
export function toComparators(range: string | Range, optionsOrLoose?: boolean | Options): string[][] {
...
function replaceTilde(comp: string, options: Options): string {
    ...
    return comp.replace(r, (_: string, M: string, m: string, p: string, pr: string) => {
        let ret: string;
        if (isX(M)) {
            ret = "";
        }
...
// 1.2 - 3.4 => >=1.2.0 <3.5.0
function hyphenReplace($0: any, from: any, fM: any, fm: any, fp: any, fpr: any, fb: any, to: any, tM: any, tm: any, tp: any, tpr: any, tb: any) {
    if (isX(fM)) {
        from = "";
    }
    else if (isX(fm)) {
...
export function validRange(range: string | Range | null, optionsOrLoose?: boolean | Options): string | null {
    try {
        if (range === null)
            return null;
...
export function prerelease(version: string | SemVer, optionsOrLoose?: boolean | Options): ReadonlyArray<string | number> | null {
 ...
 */
export function intersects(range1: string | Range | Comparator, range2: string | Range | Comparator, optionsOrLoose?: boolean | Options): boolean {
 ...
 */
export function coerce(version: string | SemVer, optionsOrLoose?: boolean | Options): SemVer | null {
    if (version instanceof SemVer) {
        return version;
    }
    if (typeof version !== "string") {
