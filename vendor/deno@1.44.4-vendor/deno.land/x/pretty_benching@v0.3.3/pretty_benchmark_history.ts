...
export class prettyBenchmarkHistory<T = unknown, K = unknown> {
    ...
    previousData?: BenchmarkHistory<T, K>, options?: prettyBenchmarkHistoryOptions<T, K>) {
    ...
    addResults(runResults: BenchmarkRunResult, options?: {
        ...
        if (this.options?.minRequiredRuns) {
            const notEnoughRuns = runResults.results.filter((r) => r.runsCount < this.options?.minRequiredRuns! ||
                r.measuredRunsMs.length < this.options?.minRequiredRuns!);
            if (notEnoughRuns.length !== 0) {
                throw new Error(`Minimum required runs (${this.options
                    ?.minRequiredRuns}) was not fullfilled by benchmarks: [${notEnoughRuns.map((r) => `"${r.name}" (${r.runsCount})`).join(", ")}]. The minimum required runs can be set with 'minRequiredRuns' option.`);
            }
        }
        if (!this.options?.easeOnlyHrTime) {
            ...
            if (runResults.results.some((r) => !isHrTime(r.totalMs))) { // TODO consider: check on a subset of measurements too.
                throw new Error(`Seems like you are trying to add results, that were measured without the --allow-hrtime flag. You can bypass this check with the 'easeOnlyHrTime' option.`);
            }
        }
    ...
    getDeltasFrom(results: BenchmarkRunResult, keys: DeltaKey<T>[] = ["measuredRunsAvgMs", "totalMs"]): {
        [key: string]: {
            [key: string]: Delta;
        };
    } {
        ...
        keys.forEach((key) => {
            if (key === "measuredRunsAvgMs" || key === "totalMs") {
                ...
                if (!currentResultExtras || typeof currentResultExtras[key] === undefined) {
                    throw new Error(`No property named "${String(key)}" in calculated extras for the currently measured benchmark named "${result.name}".`);
                }
    ...
    getBenchmarkNames() {
        return [
            ...new Set(this.data.history.map((h) => Object.keys(h.benchmarks)).flat()),
        ];
    }
}
...
export function calculateThresholds<T, K>(history: prettyBenchmarkHistory<T, K>, options?: {
