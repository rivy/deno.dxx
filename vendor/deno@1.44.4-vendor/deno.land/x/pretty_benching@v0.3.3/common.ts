...
export function getTimeColor(name: string, time: number, nocolor?: boolean, thresholds?: Thresholds) {
...
export function getInThresholdRange(name: string, time: number, thresholds?: Thresholds): null | 1 | 2 | 3 {
...
export function getIndicator(name: string, indicators?: BenchIndicator[]) {
...
export function getPaddedIndicator(name: string, toLength: number, indicators?: BenchIndicator[], noIndicator: string = " ".repeat(toLength)) {
    const indicator = getIndicator(name, indicators);
    if (indicator) {
        let newIndicator = "";
        if (typeof indicator === "object" && indicator.indicator &&
            !isNaN(indicator.visibleLength)) {
            newIndicator = padStartVisible(`${indicator.indicator}`, toLength +
...
export function calculateStdDeviation(result: BenchmarkResult) {
    const sorted = [...result.measuredRunsMs].sort();
    const stdDeviation = Math.sqrt(sorted.map((x) => Math.pow(x - result.measuredRunsAvgMs, 2)).reduce((a, b) => a + b) / sorted.length);
    return stdDeviation;
}
/** Returns the range into which the benchmarks with had a threshold set fell. */
export function getThresholdResultsFrom(runResult: BenchmarkRunResult, thresholds: Thresholds) {
    const thResults: {
