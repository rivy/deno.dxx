import { calculateExtraMetrics, calculateStdDeviation, getInThresholdRange, getPaddedIndicator, stripColor, } from "./common.ts";
...
function _prettyBenchmarkDown(runResult: BenchmarkRunResult, outputFn: (out: string) => void, options?: prettyBenchmarkDownOptions) {
    ...
    if (options?.groups && options.groups.length > 0) {
        ...
        const unmatched: GroupDefinition & {
            items: BenchmarkResult[];
        } = {
...
export function thresholdsColumn(thresholds: Thresholds, indicateResult?: boolean) {
    return {
        title: "Thresholds",
        align: "right",
        formatter: (result: BenchmarkResult, cd: ColumnDefinition) => {
 ...
 * ```
 * . */
export function extraMetricsColumns(options?: {
