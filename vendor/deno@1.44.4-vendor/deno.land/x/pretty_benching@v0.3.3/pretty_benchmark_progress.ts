...
function _prettyBenchmarkProgress(progress: BenchmarkRunProgress, options?: prettyBenchmarkProgressOptions) {
...
function considerPrecise(result: BenchmarkRunResult) {
    if (!usingHrTime() &&
        ...
        console.log(`${yellowHeader} Consider running benchmarks with ${c.yellow(`--allow-hrtime`)} for a more precise measurement`);
    }
}
function startingBenchmarkLine(progress: BenchmarkRunProgress, options?: prettyBenchmarkProgressOptions): string {
    const fullName = benchNameFormatted(progress.running!.name, options);
    const fullTimes = `[${c.yellow(progress.running!.runsCount.toString().padStart(7))}]`;
    return `Running ${fullName} a total of ${fullTimes} times`;
}
function runningBenchmarkLine(progress: BenchmarkRunProgress, options?: prettyBenchmarkProgressOptions): string {
    const percent = Math.round(progress.running!.measuredRunsMs.length / progress.running!.runsCount * 100);
    const fullName = benchNameFormatted(progress.running!.name, options);
    ...
    const inserted = progressBar.substr(0, maxBarLength / 2 - 2) +
        c.white(`${percent.toString().padEnd(2)}${percent == 100 ? "" : c.green(progressBar.substr(maxBarLength / 2, 1))}%`) + progressBar.substr(maxBarLength / 2 + 2);
    const fullProgressBar = `${c.yellow("[")}${c.green(inserted)}${c.yellow("]")}`;
    const progressDone = `${progress.running!.measuredRunsMs.length.toString().padStart(6)}`;
...
function finishedBenchmarkLine(progress: BenchmarkRunProgress, options?: prettyBenchmarkProgressOptions): string {
    ...
    const colorFn = getTimeColor(result.name, avgTime, options?.nocolor, options?.thresholds);
...
function startBenchingLine(progress: BenchmarkRunProgress, options?: prettyBenchmarkProgressOptions): string {
    const cyanHeader = `${c.cyan(headerPadding)}`;
    const fullQueued = `Benchmarks queued: [${c.yellow(progress.queued!.length.toString().padStart(5))}]`;
    const fullFiltered = c.gray(` filtered: [${progress.filtered.toString().padStart(5)}]`);
...
function benchNameFormatted(name: string, options?: prettyBenchmarkProgressOptions) {
    ...
    return `${getPaddedIndicator(name, options?.indicators ? 2 : 0, options?.indicators)}` +
        `${ob}${c.cyan(name)} ${c.gray(padEndVisible("", 40 - name.length, "-"))}${clb}`;
}
