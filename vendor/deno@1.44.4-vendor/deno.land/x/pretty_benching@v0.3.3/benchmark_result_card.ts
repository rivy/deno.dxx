...
let c: Colorer;
export function getResultCard(result: BenchmarkResult, colorer: Colorer, options?: prettyBenchmarkCardResultOptions) {
    c = colorer;
    ...
    table = table.replace(indPlaceholder, getPaddedIndicator(result.name, indPlaceholder.length - 1, options?.indicators) + " ");
    return table;
}
function prettyBenchmarkHeader(tb: TableBuilder, r: BenchmarkResult, options: prettyBenchmarkCardResultOptions) {
...
function prettyBenchmarkSingleRunMetrics(tb: TableBuilder, result: BenchmarkResult, options: prettyBenchmarkCardResultOptions) {
    ...
    const timeColor = getTimeColor(result.name, result.totalMs, options.nocolor, options.thresholds);
    const totalMS = `Total time: ${padEndVisible(`${timeColor(rtime(result.totalMs, 4))} ms`, 16)}`;
    tb.cellLine(`${tab}${totalRuns}`, `  ${totalMS}`, "");
    tb.separator();
}
function prettyBenchmarkThresholdLine(tb: TableBuilder, result: BenchmarkResult, options: prettyBenchmarkCardResultOptions) {
    const threshold = options.thresholds && options.thresholds[result.name];
    if (threshold) {
        const sep = "=".repeat(10);
        tb.line(`${tab}Thresholds:  ${c.green(`0 ${sep} ${rtime(threshold.green)}`)} ${c.yellow(`${sep} ${rtime(threshold.yellow)}`)} ${c.red(`${sep} ∞`)}`);
        tb.separator();
    }
}
function prettyBenchmarkMultipleRunMetrics(tb: TableBuilder, result: BenchmarkResult, options: prettyBenchmarkCardResultOptions) {
    const totalRuns = `Total runs: ${padEndVisible(c.yellow((result.runsCount).toString()), 7)}`;
    const totalMS = `Total time: ${padEndVisible(`${c.yellow(rtime(result.totalMs, 4))} ms`, 16)}`;
    const timeColor = getTimeColor(result.name, result.measuredRunsAvgMs, options.nocolor, options.thresholds);
    const avgRun = `Avg time: ${padEndVisible(`${timeColor(rtime(result.measuredRunsAvgMs, 4))} ms`, 8)}`;
    tb.cellLine(`${tab}${totalRuns}`, `  ${totalMS}`, `  ${avgRun}`);
    tb.separator();
}
function prettyBenchmarkMultipleRunCalcedMetrics(tb: TableBuilder, result: BenchmarkResult, options: prettyBenchmarkCardResultOptions) {
    const { max, min, mean, median } = calculateExtraMetrics(result);
    const minColor = getTimeColor(result.name, min, options.nocolor, options.thresholds);
    const maxColor = getTimeColor(result.name, max, options.nocolor, options.thresholds);
    const meanColor = getTimeColor(result.name, mean, options.nocolor, options.thresholds);
    const medianColor = getTimeColor(result.name, median, options.nocolor, options.thresholds);
    tb.cellLine(`${tab}min: ${minColor(timeStr(min))} `, ` max: ${maxColor(timeStr(max))} `, ` mean: ${meanColor(timeStr(mean))} `, ` median: ${medianColor(timeStr(median))} `);
    tb.separator();
}
function prettyBenchmarkMultipleRunGraph(tb: TableBuilder, result: BenchmarkResult, options: prettyBenchmarkCardResultOptions) {
    ...
    r.forEach((r: number, i: number) => {
        ...
        const colorFn = getTimeColor(result.name, groupHead, options.nocolor, options.thresholds);
        const fullBar = colorFn(bar);
        const count = r.toString().padStart(6);
        const percent = perc(rp).padStart(4) + "%";
        const barHeader = ` ${padStartVisible(`${rtime(groupHead)} ms`, Math.max(rtime(max).length, 12))} _[${count}][${percent}] `;
