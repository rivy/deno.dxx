...
export class TableBuilder {
    ...
    private getHSeparator(topLine?: string, bottomLine?: string) {
        ...
        inter.forEach((ind: number) => !this.isCap(ind) && crosses.push({ i: ind, t: chars.midmid }));
        topd.forEach((ind: number) => !this.isCap(ind) && crosses.push({ i: ind, t: chars.bottommid }));
        bottomd.forEach((ind: number) => !this.isCap(ind) && crosses.push({ i: ind, t: chars.topmid }));
        return this.tableLine(undefined, crosses, lineBase);
    }
    private tableLine(content?: string, crosses?: {
        i: number;
        t: chars;
    }[], chars: charset = tableLinecharset()) {
        const line = padEndVisible(`${this.colorFn(chars.start)}${content ||
            this.colorFn(chars.line.repeat(this.width))}`, this.width + 1) + `${this.colorFn(chars.stop)}\n`;
