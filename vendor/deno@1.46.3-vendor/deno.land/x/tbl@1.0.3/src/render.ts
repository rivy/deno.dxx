...
import iro, { bold } from "../../../../deno.land/x/iro@1.0.1/src/iro.ts";
...
class Renderer {
    ...
    items.forEach((item, idx) => {
      if (item.length > this.count) {
        console.warn(
          `Row no. ${
            idx + 1
          } contains more cells than the first row. The additional cells will be truncated.`,
        );
      }
      if (item.length < this.count) {
        throw new Error(
          `Row no. ${idx + 1} has fewer cells than the first row.`,
        );
      }
      item
        .slice(0, this.count)
  ...
  public renderRow(
    row: GenericType[],
    isHeader?: boolean,
    renderVerticalSeparator?: boolean,
  ): string {
    ...
    str += row
      ...
      .map((cell, idx) => {
        ...
        return isHeader ? iro(cell, bold) : cell;
      })
