declare type IsStringLiteral<Type> = Type extends string
  ? (string extends Type ? false : true)
  : false;
declare type WordInCamelCase<Type, Word extends string = ""> = Type extends
  `${Word}${infer NextCharacter}${infer _}`
  ? NextCharacter extends Capitalize<NextCharacter> ? Word
  : WordInCamelCase<Type, `${Word}${NextCharacter}`>
  : Word;
declare type Separator = "_" | "-";
declare type IncludesSeparator<Type> = Type extends
  `${string}${Separator}${string}` ? true : false;
declare type IsOneWord<Type> = Type extends Lowercase<Type & string> ? true
  : Type extends Uppercase<Type & string> ? true
  : false;
declare type IsCamelCase<Type> = Type extends Uncapitalize<Type & string> ? true
  : false;
declare type IsPascalCase<Type> = Type extends Capitalize<Type & string> ? true
  : false;
