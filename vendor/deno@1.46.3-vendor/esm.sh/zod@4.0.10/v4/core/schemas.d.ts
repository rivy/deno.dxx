...
import { version } from "./versions.d.ts";
...
export interface $ZodNanoIDInternals
...
export interface $ZodISODateInternals
...
export interface $ZodISOTimeInternals
...
export interface $ZodISODurationInternals
...
export interface $ZodCIDRv4Internals
...
export interface $ZodCIDRv6Internals
...
export interface $ZodBase64Internals
...
export interface $ZodBase64URLInternals
...
export declare function isValidJWT(
  token: string,
  algorithm?: util.JWTAlgorithm | null,
): boolean;
...
export declare const $ZodCustomStringFormat: core.$constructor<
...
export interface $ZodNumberInternals<Input = unknown>
...
export interface $ZodNumberFormatInternals
...
export interface $ZodBooleanInternals<T = unknown>
...
export interface $ZodBigIntInternals<T = unknown>
...
export interface $ZodBigIntFormatInternals
...
export interface $ZodUndefinedInternals
...
export interface $ZodUnknownInternals
...
export interface $ZodDateInternals<T = unknown>
...
export interface $ZodArrayDef<T extends SomeType = $ZodType>
  extends $ZodTypeDef {
  type: "array";
  element: T;
}
...
> = string extends keyof T
  ? util.IsAny<T[keyof T]> extends true ? Record<string, unknown>
  ...
  : util.Prettify<
    ...
    & {
      -readonly [
        k in keyof T as T[k] extends OptionalOutSchema ? k : never
      ]?: T[k]["_zod"]["output"];
    }
    & Extra
  >;
...
> = string extends keyof T
  ...
  : util.Prettify<
    ...
    & {
      -readonly [
        k in keyof T as T[k] extends OptionalInSchema ? k : never
      ]?: T[k]["_zod"]["input"];
    }
    & Extra
  >;
...
export type $InferUnionOutput<T extends SomeType> = T extends any
  ? core.output<T>
  : never;
export type $InferUnionInput<T extends SomeType> = T extends any ? core.input<T>
  : never;
...
type IsOptionalIn<T extends SomeType> = T extends OptionalInSchema ? true
  : false;
type IsOptionalOut<T extends SomeType> = T extends OptionalOutSchema ? true
  : false;
...
> extends _$ZodTypeInternals {
  ...
  optin: IsOptionalIn<T[number]> extends false ? "optional" | undefined
    : "optional";
  optout: IsOptionalOut<T[number]> extends false ? "optional" | undefined
    : "optional";
}
...
export declare const $ZodDiscriminatedUnion: core.$constructor<
...
type TupleInputTypeWithOptionals<T extends util.TupleItems> = T extends
  readonly [
...
type TupleOutputTypeWithOptionals<T extends util.TupleItems> = T extends
  readonly [
    ...infer Prefix extends SomeType[],
    infer Tail extends SomeType,
...
> extends
  ...
  isst:
    ...
    | errors.$ZodIssueTooBig<unknown[]>
    | errors.$ZodIssueTooSmall<unknown[]>;
}
...
export type $ZodRecordKey = $ZodType<
...
export type $InferZodRecordOutput<
  Key extends $ZodRecordKey = $ZodRecordKey,
  Value extends SomeType = $ZodType,
> = Key extends $partial ? Partial<Record<core.output<Key>, core.output<Value>>>
  : Record<core.output<Key>, core.output<Value>>;
export type $InferZodRecordInput<
  Key extends $ZodRecordKey = $ZodRecordKey,
  Value extends SomeType = $ZodType,
> = Key extends $partial ? Partial<Record<core.input<Key>, core.input<Value>>>
...
export interface $ZodTransformDef extends $ZodTypeDef {
  ...
  transform: (
    input: unknown,
    payload: ParsePayload<unknown>,
  ) => util.MaybeAsync<unknown>;
}
export interface $ZodTransformInternals<O = unknown, I = unknown>
...
export interface $ZodOptionalDef<T extends SomeType = $ZodType>
  extends $ZodTypeDef {
  type: "optional";
  innerType: T;
}
export interface $ZodOptionalInternals<T extends SomeType = $ZodType>
  extends
    $ZodTypeInternals<core.output<T> | undefined, core.input<T> | undefined> {
...
export interface $ZodNullableDef<T extends SomeType = $ZodType>
  extends $ZodTypeDef {
  type: "nullable";
  innerType: T;
}
...
export interface $ZodDefaultDef<T extends SomeType = $ZodType>
  extends $ZodTypeDef {
  type: "default";
  innerType: T;
...
export interface $ZodPrefaultDef<T extends SomeType = $ZodType>
  extends $ZodTypeDef {
  type: "prefault";
  innerType: T;
  /** The default value. May be a getter. */
...
export interface $ZodNonOptionalDef<T extends SomeType = $ZodType>
  extends $ZodTypeDef {
  type: "nonoptional";
  innerType: T;
}
...
export interface $ZodSuccessDef<T extends SomeType = $ZodType>
  extends $ZodTypeDef {
  type: "success";
  innerType: T;
}
export interface $ZodSuccessInternals<T extends SomeType = $ZodType>
...
export interface $ZodCatchDef<T extends SomeType = $ZodType>
  extends $ZodTypeDef {
  type: "catch";
  innerType: T;
...
export interface $ZodReadonlyDef<T extends SomeType = $ZodType>
  extends $ZodTypeDef {
  type: "readonly";
  innerType: T;
}
export interface $ZodReadonlyInternals<T extends SomeType = $ZodType>
  extends
    $ZodTypeInternals<
      util.MakeReadonly<core.output<T>>,
      util.MakeReadonly<core.input<T>>
    > {
...
> = Suffix extends LiteralPart ? `${Template}${UndefinedToEmptyString<Suffix>}`
  : Suffix extends $ZodType
    ? `${Template}${core.output<Suffix> extends infer T extends LiteralPart
...
] ? Rest extends string[] ? First extends "" ? ConcatenateTupleOfStrings<Rest>
...
export declare const $ZodTemplateLiteral: core.$constructor<
  $ZodTemplateLiteral
>;
export interface $ZodPromiseDef<T extends SomeType = $ZodType>
  extends $ZodTypeDef {
  type: "promise";
  innerType: T;
}
...
export interface $ZodLazyDef<T extends SomeType = $ZodType>
  extends $ZodTypeDef {
  type: "lazy";
  getter: () => T;
}
...
export interface $ZodCustomDef<O = unknown>
