...
import { $ZodTuple } from "./schemas.d.ts";
import type * as util from "./util.d.ts";
...
> {
  ...
  ) => ReturnType<F> extends ReturnType<this["_output"]> ? ReturnType<F>
    : ReturnType<this["_output"]>;
  implementAsync<F extends $InferInnerFunctionTypeAsync<Args, Returns>>(
    func: F,
  ): F extends $InferOuterFunctionTypeAsync<Args, Returns> ? F
  ...
  input<NewArgs extends $ZodFunctionIn>(
    args: NewArgs,
  ): $ZodFunction<NewArgs, Returns>;
  output<NewReturns extends schemas.$ZodType>(
    output: NewReturns,
  ): $ZodFunction<Args, NewReturns>;
}
export interface $ZodFunctionParams<
  I extends $ZodFunctionIn,
  O extends schemas.$ZodType,
> {
  input?: I;
  output?: O;
}
...
declare function _function<const In extends $ZodFunctionIn = $ZodFunctionIn>(
  params: {
    input: In;
  },
): $ZodFunction<In, $ZodFunctionOut>;
declare function _function<const Out extends $ZodFunctionOut = $ZodFunctionOut>(
  params: {
    output: Out;
  },
