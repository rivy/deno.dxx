import type * as checks from "./checks.d.ts";
import type { $ZodConfig } from "./core.d.ts";
...
export type OmitIndexSignature<T> = {
  [K in keyof T as string extends K ? never : K extends string ? K : never]:
    T[K];
};
...
export declare function assertEqual<A, B>(
  val: AssertEqual<A, B>,
): AssertEqual<A, B>;
export declare function assertNotEqual<A, B>(
...
export declare function joinValues<T extends Primitive[]>(
  array: T,
  separator?: string,
): string;
...
export declare function promiseAllObject<T extends object>(
  promisesObj: T,
): Promise<
  {
...
export declare function clone<T extends schemas.$ZodType>(
  inst: T,
  def?: T["_zod"]["def"],
  params?: {
    parent: boolean;
...
export type Normalize<T> = T extends undefined ? never
  : T extends Record<any, any> ? Flatten<
      & {
...
export declare function createTransparentProxy<T extends object>(
  getter: () => T,
): T;
...
export type CleanKey<T extends PropertyKey> = T extends `?${infer K}` ? K
  : T extends `${infer K}?` ? K
  : T;
...
export type FromCleanMap<T extends schemas.$ZodLooseShape> = {
  [
    k in keyof T as k extends `?${infer K}` ? K
      : k extends `${infer K}?` ? K
      : k
  ]: k;
};
...
export declare function aborted(
...
export declare function unwrapMessage(
  message:
    | string
    | {
      message: string;
...
export declare function finalizeIssue(
  iss: errors.$ZodRawIssue,
  ctx: schemas.ParseContextInternal | undefined,
  config: $ZodConfig,
): errors.$ZodIssue;
export declare function getSizableOrigin(
  input: any,
