...
import {
	...
	intoPath,
	intoURL,
	logger as log, /* note: initialized to the suspended state */
	traversal,
	validURL,
...
const possibleLogLevelsInOrder =
	([defaultLogLevel, logLevelFromEnv, ...logLevelsFromOptions, ...logLevelsFromLogLevelArgv,]
		.filter((e,) => log.logLevelDetail(e,)) as string[])
		.reverse();
...
log.trace(
	{ SOURCE, },
	'=>',
	source instanceof Uint8Array ? `Uint8Array[${source.length}]` : source,
);
