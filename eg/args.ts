...
const YargsParserConfigDefault = {
	// * (boilerplate) usual parser options
	'camel-case-expansion': false, // disable automatic camelCase aliases for hyphenated options (only within generated Yargs parse result object)
...
const yargsParserConfig = {
	...
	...{
		...
		'unknown-options-as-args': false, // do not treat unknown options as arguments (instead, display errors)
	},
};
...
type AppInfo = {
	...
	epilog?: string;
	runAs?: string;
	yargsParserConfig: typeof yargsParserConfig;
};

const appInfo: AppInfo = {
	$me,
	description: 'Display all arguments.',
	copyright: '* Copyright (c) 2021-2024+ * Roy Ivy III (MIT license)',
	version: $version.v(),
	yargsParserConfig,
};
...
function yargsAppBoilerplate(info: AppInfo) {
	const appName = info.name ?? info.$me.name;
	const appDescription = info.description;
	const appCopyright = info.copyright;
	const appEpilog = info.epilog;
	const appVersion = info.version;
	const appRunAs = info.runAs ?? info.$me.runAs;

	// ref: <https://devhints.io/yargs> , <https://github.com/yargs/yargs/tree/v17.0.1-deno/docs>
	let yargsInstance = $yargs(/* argv */ undefined, /* cwd */ undefined)
...
Usage:\n  ${appRunAs} [OPTION..] [ARG..]`,
	...
	if (appEpilog != null) {
		yargsInstance = yargsInstance.epilog(appEpilog);
		yargsInstance = yargsInstance.epilog('');
	}
	if (appCopyright != null) {
		yargsInstance = yargsInstance.epilog(appCopyright);
	}
	...
	yargsInstance = yargsInstance
		...
		.option('trace', {
			describe: `Set 'trace' (high-detail 'debug') level logging`,
			type: 'boolean',
		})
		// * (boilerplate) meta options
		.option('_is_compiled_binary_', {
			// * workaround to determine whether process is a standalone compile binary (see <https://github.com/denoland/deno/issues/15996> @@ <https://archive.is/wQJN5> @@ <https://web.archive.org/web/20260919214235/<URL>>)
			describe: `Claim that this process is a standalone compiled binary`,
			type: 'boolean',
			hidden: true,
		})
		// * (boilerplate) configure Options, Logging, Help/Info, and Hidden groups
		.group([], 'Options:')
		.group(['log-level', 'silent', 'quiet', 'verbose', 'debug', 'trace'], '*Logging:')
		.group(['help', 'version'], '*Help/Info:')
		.group(['_is_compiled_binary_'], '*Hidden:');

	if (appInfo.yargsParserConfig['camel-case-expansion']) {
		yargsInstance = yargsInstance.alias('_is_compiled_binary_', 'isCompiledBinary');
	}
	// if ((appInfo.yargsParserConfig.'camel-case-expansion' ?? false)

	// // * Yargs parser configuration
	// // ref: [Yargs Parser ~ Configuration](https://github.com/yargs/yargs-parser#configuration)
	// .parserConfiguration(YargsParserConfigDefault);

	return yargsInstance;
}

// ref: <https://devhints.io/yargs> , <https://github.com/yargs/yargs/tree/v17.0.1-deno/docs>
const app = yargsAppBoilerplate(appInfo)
	.parserConfiguration({
		...YargsParserConfigDefault,
		...{
			// * per app configuration options
	...
	/* Options... */
	.strictOptions(/* enable */ true) // report errors for unknown options
...
const argvMetaKeys = ['_', '$0', '_is_compiled_binary_'];
...
if (argv.help) {
	...
	const onlyHelp =
		argv._.length === 0 &&
		Object.keys(argv).filter((s) => !['help', ...argvMetaKeys].includes(s)).length === 0;
...
if (argv.version) {
	...
	const onlyVersion =
		argv._.length === 0 &&
		Object.keys(argv).filter((s) => !['version', ...argvMetaKeys].includes(s)).length === 0;
